...
TEST_CASE(cli_json_format_matches_what_corpussearch_finds_directly) {
    ...
    CHECK_EQ(parsed.size(), std::size_t{51}); // 6+8+4+9+6+6+6+6 soprano fermatas across the 8 fixture chorales, see test_corpussearch.cpp
...
TEST_CASE(cli_table_format_has_a_header_plus_one_row_per_match) {
    ...
    CHECK_EQ(lineCount, std::ptrdiff_t{52}); // 1 header + 51 matches
}
